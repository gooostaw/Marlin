...
#include "src/inc/mvCNCConfigPre.h"
  ...
  #include "src/module/pwm_temp_io.h"
  #include "src/module/stepper.h"

  #include "src/gcode/parser.h"  // for units (and volumetric)

  #if ENABLED(LCD_SHOW_E_TOTAL)
    #include "src/mvCNCCore.h"  // for jobIsActive()
  #endif

  #if ENABLED(FILAMENT_LCD_DISPLAY)
    #include "src/feature/filwidth.h"
    ...
    #include "src/feature/cooler.h"
  #endif

  #if DO_DRAW_AMMETER
    #include "src/feature/ammeter.h"
  #endif

  #if HAS_POWER_MONITOR
    #include "src/feature/power_monitor.h"
  #endif
    ...
    #include "src/module/jobcounter.h"
  #endif

  #if HAS_DUAL_MIXING
    #include "src/feature/mixing.h"
  #endif
...
FORCE_INLINE void _draw_maxis_value(const AxisEnum axis, const char *value, const bool blink) {
  ...
  const AxisEnum a   = axis;
  const uint8_t offs = a * (is_inch ? XYZ_SPACING_IN : XYZ_SPACING);
  lcd_put_wchar((is_inch ? X_LABEL_POS_IN : X_LABEL_POS) + offs, XYZ_BASELINE - 9, axis_codes[axis]);
  lcd_moveto((is_inch ? X_VALUE_POS_IN : X_VALUE_POS) + offs, XYZ_BASELINE - 9);
...
void mvCNCUI::draw_status_screen() {
  ...
  lcd_put_u8str(mystring);
  lcd_put_u8str(0, 32, Z_STR);
  lcd_put_u8str(mzstring);

  lcd_put_u8str(40, 7, "| Wpos");
  lcd_put_u8str(40, 16, "|");
  ...
  if (cutter.isReady) {
    ...
    if (cutter.unitPower) {
      lcd_put_u8str(ftostr51rj(float(cutter.unitPower) / 1000));
      lcd_put_wchar('k');
    } else {
      lcd_put_u8str("Off");
    }
  ...
  #if HAS_FAN
  for (uint8_t i = 0; NUM_M106_FANS > i; ++i) {
    if (status_row < ((progress_string[0] == '\0') ? 46 : 37)) {
      {  // Limit var scope
        char c         = '%';
        uint16_t speed = fanManager.fan_speed[i];
        if (speed) {  // Only list fans which are spinning
    #if ENABLED(ADAPTIVE_FAN_SLOWING)
          if (!blink && fanManager.fan_speed_scaler[i] < 128) {
            speed = fanManager.scaledFanSpeed(i, speed);
            c     = '*';
          }
    #endif
