 ...
 *
 *   G26 is a Mesh Validation Tool intended to provide support for the mvCNC Unified Bed Leveling System.
 *   In order to fully utilize and benefit from the mvCNC Unified Bed Leveling System an accurate Mesh must
...
typedef struct {
  ...
  bool prime_nozzle() {
    ...
    #if HAS_mvCNCUI_MENU && !HAS_TOUCH_BUTTONS // ui.button_pressed issue with touchscreen
  ...
  do {
    ...
    if (location.valid()) {
      ...
      const bool f = st.y == 0,
        ...
        if (TERN0(HAS_mvCNCUI_MENU, user_canceled())) goto LEAVE; // Check if the user wants to stop the Mesh Validation
      ...
      if (TERN0(HAS_mvCNCUI_MENU, user_canceled())) goto LEAVE;
    }
