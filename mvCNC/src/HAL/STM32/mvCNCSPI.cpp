...
void mvCNCSPI::setClockDivider(uint8_t _div) {
  _speed = spi_getClkFreq(&_spi);// / _div;
  _clockDivider = _div;
}

void mvCNCSPI::begin(void) {
  //TODO: only call spi_init if any parameter changed!!
  spi_init(&_spi, _speed, _dataMode, _bitOrder, _dataSize);
}

void mvCNCSPI::setupDma(SPI_HandleTypeDef &_spiHandle, DMA_HandleTypeDef &_dmaHandle, uint32_t direction, bool minc) {
...
byte mvCNCSPI::transfer(uint8_t _data) {
  uint8_t rxData = 0xFF;
...
uint8_t mvCNCSPI::dmaTransfer(const void *transmitBuf, void *receiveBuf, uint16_t length) {
...
uint8_t mvCNCSPI::dmaSend(const void * transmitBuf, uint16_t length, bool minc) {
