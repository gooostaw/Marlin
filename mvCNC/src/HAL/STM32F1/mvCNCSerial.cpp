...
// Not every mvCNCSerial port should handle emergency parsing.
...
#define DEFINE_HWSERIAL_mvCNC(name, n)     \
...
#define DEFINE_HWSERIAL_UART_mvCNC(name, n) \
  MSerialT name(serial_handles_emergency(n), \
          UART##n,                           \
          BOARD_USART##n##_TX_PIN,           \
          BOARD_USART##n##_RX_PIN);          \
...
// port which may be in use on the system.
  ...
  DEFINE_HWSERIAL_mvCNC(MSerial1, 1);
#endif
DEFINE_HWSERIAL_mvCNC(MSerial2, 2);
DEFINE_HWSERIAL_mvCNC(MSerial3, 3);
#if EITHER(STM32_HIGH_DENSITY, STM32_XL_DENSITY)
  DEFINE_HWSERIAL_UART_mvCNC(MSerial4, 4);
  DEFINE_HWSERIAL_UART_mvCNC(MSerial5, 5);
#endif
