...
void Timer::setCompare(uint32_t compare) {
  ...
  if (active) {
    ...
    nsec_offset = nsec_offset < 1000 ? nsec_offset : 0; // constrain, this shouldn't be needed but apparently mvCNC enables interrupts on the stepper timer before initialising it, todo: investigate ?bug?
  }
