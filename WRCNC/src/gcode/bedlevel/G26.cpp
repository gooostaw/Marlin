 ...
 *
 *   G26 is a Mesh Validation Tool intended to provide support for the WRCNC Unified Bed Leveling System.
 *   In order to fully utilize and benefit from the WRCNC Unified Bed Leveling System an accurate Mesh must
...
typedef struct {
  ...
  bool turn_on_heaters() {
    ...
    SERIAL_ECHOLNPGM("Waiting for heatup.");
      ...
      if (bed_temp > 25) {
          ...
          ui.quick_feedback();
          TERN_(HAS_WRCNCUI_MENU, ui.capture());
        #endif
  ...
  bool prime_nozzle() {
    ...
    #if HAS_WRCNCUI_MENU && !HAS_TOUCH_BUTTONS // ui.button_pressed issue with touchscreen
...
void GcodeSuite::G26() {
  ...
  do {
    ...
    if (location.valid()) {
      ...
      const bool f = st.y == 0,
        ...
        if (TERN0(HAS_WRCNCUI_MENU, user_canceled())) goto LEAVE; // Check if the user wants to stop the Mesh Validation
      ...
      if (TERN0(HAS_WRCNCUI_MENU, user_canceled())) goto LEAVE;
    }
  ...
  TERN_(HAS_WRCNCUI_MENU, ui.release()); // Give back control of the LCD
