 ...
 *
 * Using Workspace Offsets (default WRCNC behavior):
...
void GcodeSuite::G92() {
  ...
  switch (subcode_G92) {
    default: return;                                                  // Ignore unknown G92.x
      ...
      case 9:                                                         // G92.9 - Set Current Position directly (like WRCNC 1.0)
        LOOP_LOGICAL_AXES(i) {
      ...
      LOOP_LOGICAL_AXES(i) {
        if (parser.seenval(axis_codes[i])) {
          ...
          if (!NEAR_ZERO(d)) {
              ...
              current_position[i] = v;                                // ...set Current Position directly (like WRCNC 1.0)
            #endif
          }
        }
      }
