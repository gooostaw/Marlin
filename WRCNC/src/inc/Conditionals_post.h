 ...
 */
  ...
  // Set SD_DETECT_STATE based on hardware if not overridden
    ...
    #if BOTH(HAS_WRCNCUI_MENU, ELB_FULL_GRAPHIC_CONTROLLER) && (SD_CONNECTION_IS(LCD) || !defined(SDCARD_CONNECTION))
...
#if ANY(HAS_GRAPHICAL_TFT, LCD_USE_DMA_FSMC, HAS_FSMC_GRAPHICAL_TFT, HAS_SPI_GRAPHICAL_TFT, IS_DWIN_WRCNCUI) || !PIN_EXISTS(SD_DETECT)
...
#if HAS_TEMPERATURE && EITHER(HAS_WRCNCUI_MENU, HAS_DWIN_E3V2)
 ...
 */
  ...
  #if ANY(HAS_WRCNCUI_MENU, EXTENSIBLE_UI, DWIN_CREALITY_LCD_ENHANCED, DWIN_CREALITY_LCD_JYERSUI) || BOTH(EMERGENCY_PARSER, HOST_PROMPT_SUPPORT)
