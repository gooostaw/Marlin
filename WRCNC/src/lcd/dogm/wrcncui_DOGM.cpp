...
/**
 * lcd/dogm/wrcncui_DOGM.h
 *
 * Implementation of the LCD display routines for a DOGM128 graphic display.
 * by STB for ErikZalm/WRCNC. Common LCD 128x64 pixel graphic displays.
 *
 ...
 * (See https://wrcncfw.org/docs/development/fonts.html)
 */
#include "fontdata/fontdata_ISO10646_1.h"
#if ENABLED(USE_SMALL_INFOFONT)
  #include "fontdata/fontdata_6x9_wrcnc.h"
...
#if HAS_LCD_CONTRAST
  void WRCNCUI::_set_contrast() { u8g.setContrast(contrast); }
#endif

void WRCNCUI::set_font(const WRCNCFont font_nr) {
...
bool WRCNCUI::detected() { return true; }
    ...
    void WRCNCUI::draw_custom_bootscreen(const uint8_t frame/*=0*/) {
    ...
    void WRCNCUI::show_custom_bootscreen() {
  ...
  // Draw the static WRCNC bootscreen from a u8g loop
  // or the animated boot screen within its own u8g loop
  void WRCNCUI::draw_wrcnc_bootscreen(const bool line2/*=false*/) {

    // Determine text space needed
    constexpr u8g_uint_t text_width_1 = u8g_uint_t((sizeof(SHORT_BUILD_VERSION) - 1) * (MENU_FONT_WIDTH)),
                         text_width_2 = u8g_uint_t((sizeof(WRCNC_WEBSITE_URL) - 1) * (MENU_FONT_WIDTH)),
    ...
    auto _draw_bootscreen_bmp = [&](const uint8_t *bitmap) {
      ...
      if (!two_part || line2) lcd_put_u8str(txt_offx_2, txt_base, F(WRCNC_WEBSITE_URL));
    };
    ...
    #if DISABLED(BOOT_WRCNC_LOGO_ANIMATED)
      draw_bootscreen_bmp(start_bmp);
    #else
      constexpr millis_t frame_time = WRCNC_BOOTSCREEN_FRAME_TIME;
      LOOP_L_N(f, COUNT(wrcnc_bootscreen_animation)) {
        draw_bootscreen_bmp((uint8_t*)pgm_read_ptr(&wrcnc_bootscreen_animation[f]));
        if (frame_time) safe_delay(frame_time);
      }
    #endif
  }

  // Show the WRCNC bootscreen, with the u8g loop and delays
  void WRCNCUI::show_wrcnc_bootscreen() {
    for (uint8_t q = bootscreen_pages; q--;) {
      draw_wrcnc_bootscreen(q == 0);
...
void WRCNCUI::update_language_font() {
...
void WRCNCUI::clear_lcd() { } // Automatically cleared by Picture Loop

#if HAS_LCD_BRIGHTNESS

  void WRCNCUI::_set_brightness() {
