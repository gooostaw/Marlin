...
void ChironTFT::Startup() {
  ...
  // Filament runout is handled by WRCNC settings in Configuration.h
...
void ChironTFT::ConfirmationRequest(const char * const msg)  {
  ...
  switch (cnc_state) {
    ...
    case AC_cnc_paused: {
      ...
      else if (strcmp_P(msg, WRCNC_msg_reheat_done) == 0) {
        pause_state = AC_paused_idle;
        SendtoTFTLN(AC_msg_paused); // enable continue button
      }
      // Filament Purging, send acknowledgement enter run mode
      else if (strcmp_P(msg, WRCNC_msg_filament_purging) == 0) {
...
void ChironTFT::StatusChange(const char * const msg)  {
  ...
  switch (cnc_state) {
    case AC_cnc_probing: {
      ...
      if (strcmp_P(msg, WRCNC_msg_probing_failed) == 0) {
  ...
  if (!msg_matched) {
    if (strcmp_P(msg, WRCNC_msg_extruder_heating) == 0) {
      SendtoTFTLN(AC_msg_nozzle_heating);
      hotend_state = AC_heater_temp_set;
    }
    else if (strcmp_P(msg, WRCNC_msg_bed_heating) == 0) {
      SendtoTFTLN(AC_msg_bed_heating);
      hotbed_state = AC_heater_temp_set;
    }
    else if (strcmp_P(msg, WRCNC_msg_EEPROM_version) == 0) {
