...
namespace ExtUI {
  ...
  void enableHeater(const heater_t heater) {
    ...
    void jog(const xyz_float_t &dir) {
      ...
      // WRCNC by GCODE routines, but should remain untouched
  ...
  GcodeSuite::WRCNCBusyState getHostKeepaliveState() { return TERN0(HOST_KEEPALIVE_FEATURE, gcode.busy_state); }
  ...
  PGM_P getFirmwareName_str() {
    static PGMSTR(firmware_name, "WRCNC " SHORT_BUILD_VERSION);
...
// At the moment we hook into WRCNCUI methods, but this could be cleaned up in the future

void WRCNCUI::init_lcd() { ExtUI::onStartup(); }

void WRCNCUI::update() { ExtUI::onIdle(); }
