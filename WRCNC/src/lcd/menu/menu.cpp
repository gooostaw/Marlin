...
void WRCNCUI::return_to_status() { goto_screen(status_screen); }

void WRCNCUI::push_current_screen() {
...
void WRCNCUI::_goto_previous_screen(TERN_(TURBO_BACK_MENU_ITEM, const bool is_back/*=false*/)) {
 ...
 */
void WRCNCUI::goto_screen(screenFunc_t screen, const uint16_t encoder/*=0*/, const uint8_t top/*=0*/, const uint8_t items/*=0*/) {
  if (currentScreen != screen) {

    TERN_(IS_DWIN_WRCNCUI, did_first_redraw = false);
    ...
    screen_changed = true;
    TERN_(HAS_WRCNCUI_U8GLIB, drawing_screen = false);

    TERN_(HAS_WRCNCUI_MENU, encoder_direction_normal());
...
//
void WRCNCUI::synchronize(PGM_P const msg/*=nullptr*/) {
...
#if HAS_BUZZER
  void WRCNCUI::completion_feedback(const bool good/*=true*/) {
...
//
bool WRCNCUI::selection; // = false
bool WRCNCUI::update_selection() {
