  ...
  void _lcd_babystep(const AxisEnum axis, PGM_P const msg) {
    ...
    if (ui.should_draw()) {
      ...
      MenuEditItemBase::draw_edit_screen(msg, BABYSTEP_TO_STR(mps * babystep.accum));
        ...
        const bool in_view = TERN1(HAS_WRCNCUI_U8GLIB, PAGE_CONTAINS(LCD_PIXEL_HEIGHT - MENU_FONT_HEIGHT, LCD_PIXEL_HEIGHT - 1));
        if (in_view) {
          TERN_(HAS_WRCNCUI_U8GLIB, ui.set_font(FONT_MENU));
            ...
            lcd_moveto(10, 3);
          #else
            lcd_moveto(0, TERN(HAS_WRCNCUI_U8GLIB, LCD_PIXEL_HEIGHT - MENU_FONT_DESCENT, LCD_HEIGHT - 1));
            lcd_put_u8str(GET_TEXT_F(MSG_BABYSTEP_TOTAL));
            lcd_put_wchar(':');
          #endif
