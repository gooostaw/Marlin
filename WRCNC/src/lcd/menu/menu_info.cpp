...
#if BOTH(HAS_WRCNCUI_MENU, LCD_INFO_MENU)
...
//
  ...
  void menu_show_wrcnc_bootscreen() {
    if (ui.use_click()) { ui.goto_previous_screen_no_defer(); }
    ui.draw_wrcnc_bootscreen();
  }

  #if ENABLED(SHOW_CUSTOM_BOOTSCREEN)
    void menu_show_custom_bootscreen() {
      if (ui.use_click()) { ui.goto_screen(menu_show_wrcnc_bootscreen); }
...
void menu_info() {
  ...
  #if HAS_GAMES
  {
    ...
    SUBMENU(
      #if HAS_GAME_MENU
        MSG_GAMES, menu_game
      #elif ENABLED(WRCNC_BRICKOUT)
        MSG_BRICKOUT, brickout.enter_game
      #elif ENABLED(WRCNC_INVADERS)
        MSG_INVADERS, invaders.enter_game
      #elif ENABLED(WRCNC_SNAKE)
        MSG_SNAKE, snake.enter_game
      #elif ENABLED(WRCNC_MAZE)
...
#endif // HAS_WRCNCUI_MENU && LCD_INFO_MENU
