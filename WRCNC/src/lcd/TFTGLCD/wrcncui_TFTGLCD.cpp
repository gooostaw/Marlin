...
#if HAS_LCD_CONTRAST
  void WRCNCUI::_set_contrast() { lcd.setContrast(contrast); }
#endif
...
static void center_text_P(PGM_P pstart, uint8_t y) {
  ...
  void WRCNCUI::show_bootscreen() {
    ...
    lcd.setCursor(indent, 1); lcd.write(LR);  lcd_put_u8str(F("WRCNC"));  lcd.write(LR);
    lcd.setCursor(indent, 2); lcd.write(BLC); lcd_put_u8str(F("------"));  lcd.write(BRC);
    center_text_P(PSTR(SHORT_BUILD_VERSION), 3);
    center_text_P(PSTR(WRCNC_WEBSITE_URL), 4);
    picBits = ICON_LOGO;
    lcd.print_screen();
  }
