...
#if ANY(HAS_WRCNCUI_MENU, ULTIPANEL_FEEDMULTIPLY, SOFT_RESET_ON_KILL)
...
public:
  ...
  WRCNCUI() {
    TERN_(HAS_WRCNCUI_MENU, currentScreen = status_screen);
  }
  ...
  static void status_printf(const uint8_t level, FSTR_P const fmt, ...);
    ...
    static void flow_fault();
      ...
      FORCE_INLINE static void refresh(const LCDViewAction type) { lcdDrawUpdate = type; }
        ...
        #endif
        static void draw_wrcnc_bootscreen(const bool line2=false);
        static void show_wrcnc_bootscreen();
        static void show_bootscreen();
        static void bootscreen_completion(const millis_t sofar);
      #endif

      #if HAS_WRCNCUI_U8GLIB
        static void set_font(const WRCNCFont font_nr);
      #elif IS_DWIN_WRCNCUI
      ...
      #if ENABLED(LCD_PROGRESS_BAR) && !HAS_WRCNCUI_U8GLIB
  ...
  #if ENABLED(SDSUPPORT)
    #if BOTH(SCROLL_LONG_FILENAMES, HAS_WRCNCUI_MENU)
      #define WRCNCUI_SCROLL_NAME 1
    #endif
  ...
  #if ENABLED(ADVANCED_PAUSE_FEATURE) && ANY(HAS_WRCNCUI_MENU, EXTENSIBLE_UI, DWIN_CREALITY_LCD_ENHANCED, DWIN_CREALITY_LCD_JYERSUI)
  ...
  //
  #if HAS_WRCNCUI_MENU && EITHER(AUTO_BED_LEVELING_UBL, G26_MESH_VALIDATION)
