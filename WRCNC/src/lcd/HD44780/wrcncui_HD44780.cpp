...
void WRCNCUI::set_custom_characters(const HD44780CharSet screen_charset/*=CHARSET_INFO*/) {
  ...
  #if BOTH(SDSUPPORT, HAS_WRCNCUI_MENU)
...
bool WRCNCUI::detected() {
  return TERN1(DETECT_I2C_LCD_DEVICE, lcd.LcdDetected() == 1);
}

#if HAS_SLOW_BUTTONS
  uint8_t WRCNCUI::read_slow_buttons() {
...
void WRCNCUI::clear_lcd() { lcd.clear(); }
  ...
  static void logo_lines(PGM_P const extra) {
    ...
    lcd_put_u8str(indent, 1, F("|WRCNC|"));  lcd_put_u8str_P(extra);
  ...
  void WRCNCUI::show_bootscreen() {
    ...
    //
    // Show the WRCNC logo with splash line 1
    //
    if (LCD_EXTRA_SPACE >= utf8_strlen(SHORT_BUILD_VERSION) + 1) {
      //
      // Show the WRCNC logo, splash line1, and splash line 2
      //
      logo_lines(PSTR(" " SHORT_BUILD_VERSION));
      CENTER_OR_SCROLL(WRCNC_WEBSITE_URL, 2000);
    }
    else {
      //
      // Show the WRCNC logo and short build version
      // After a delay show the website URL
      //
      logo_lines(NUL_STR);
      CENTER_OR_SCROLL(SHORT_BUILD_VERSION, 1500);
      CENTER_OR_SCROLL(WRCNC_WEBSITE_URL, 1500);
  ...
  void WRCNCUI::bootscreen_completion(const millis_t) {
    lcd.clear();
    safe_delay(100);
...
FORCE_INLINE void _draw_bed_status(const bool blink) {
  ...
  void WRCNCUI::draw_progress_bar(const uint8_t percent) {
...
void WRCNCUI::draw_status_screen() {
  ...
  void MenuItem_confirm::draw_select_screen(PGM_P const yes, PGM_P const no, const bool yesno, PGM_P const pref, const char * const string/*=nullptr*/, PGM_P const suff/*=nullptr*/) {
    ...
    void WRCNCUI::update_indicators() {
