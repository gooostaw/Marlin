...
#include "../../../module/planner.h"
  ...
  #include "../../../WRCNCCore.h" // for printingIsActive
#endif
...
FORCE_INLINE void _draw_axis_value(const AxisEnum axis, const char *value, const bool blink, const uint16_t x, const uint16_t y) {
    ...
    if (blink)
      ...
      dwin_string.add(TERN1(DWIN_WRCNCUI_PORTRAIT, axis == Z_AXIS) ? PSTR("       ") : PSTR("    "));
    else
      dwin_string.add(value);

    // For E_TOTAL there may be some characters to cover up
    if (BOTH(DWIN_WRCNCUI_PORTRAIT, LCD_SHOW_E_TOTAL) && axis == X_AXIS)
      dwin_string.add("   ");

    DWIN_Draw_String(true, font14x28, Color_White, Color_Bg_Black, x, y + 32, S(dwin_string.string()));

  #else // !DWIN_WRCNCUI_PORTRAIT
...
/**
 * Draw the WRCNCUI Status Screen for Ender 3 V2
 */
void WRCNCUI::draw_status_screen() {
  ...
  if (!ui.did_first_redraw) {
    ...
    DWIN_ICON_Show(ICON, ICON_LOGO_WRCNC,
      #if ENABLED(DWIN_WRCNCUI_PORTRAIT)
  ...
  constexpr int16_t cpy = TERN(DWIN_WRCNCUI_PORTRAIT, 195, 117);
  if (show_e_total) {
    TERN_(LCD_SHOW_E_TOTAL, _draw_e_value(e_move_accumulator, TERN(DWIN_WRCNCUI_PORTRAIT, 6, 75), cpy));
  }
  else {
                      _draw_axis_value(X_AXIS, ftostr4sign(lpos.x), blink, TERN(DWIN_WRCNCUI_PORTRAIT,  6,  75), cpy);
    TERN_(HAS_Y_AXIS, _draw_axis_value(Y_AXIS, ftostr4sign(lpos.y), blink, TERN(DWIN_WRCNCUI_PORTRAIT, 95, 184), cpy));
  }
  TERN_(HAS_Z_AXIS, _draw_axis_value(Z_AXIS, ftostr52sp(lpos.z), blink, TERN(DWIN_WRCNCUI_PORTRAIT, 165, 300), cpy));
  ...
  constexpr int16_t pb_margin = 5,
                    pb_left = pb_margin + TERN(DWIN_WRCNCUI_PORTRAIT, 0, 90),
                    pb_height = TERN(DWIN_WRCNCUI_PORTRAIT, 60, 20),
                    pb_right = LCD_PIXEL_WIDTH - pb_margin,
                    pb_bottom = TERN(DWIN_WRCNCUI_PORTRAIT, 410, 220),
