...
// but we'll make the font modal for WRCNC
dwin_font_t dwin_font = { font8x16, 8, 16, Color_White, Color_Bg_Black, true };
void WRCNCUI::set_font(const uint8_t font_nr) {
...
bool WRCNCUI::detected() { return true; }

// Initialize or re-initialize the LCD
void WRCNCUI::init_lcd() { DWIN_Startup(); }

// This LCD should clear where it will draw anew
void WRCNCUI::clear_lcd() {
  ...
  void WRCNCUI::show_bootscreen() {
    ...
    dwin_string.set(F(SHORT_BUILD_VERSION));
      ...
      DWIN_ICON_Show(BOOT_ICON, ICON_WRCNCURL,  LOGO_CENTER - 100 / 2, 440);
      ...
      DWIN_ICON_Show(BOOT_ICON, ICON_WRCNCURL,  INFO_CENTER - 100 / 2, 152);
  ...
  void WRCNCUI::bootscreen_completion(const millis_t sofar) {
...
void WRCNCUI::draw_kill_screen() {
  ...
  #if ENABLED(DWIN_WRCNCUI_LANDSCAPE)
...
// Status Message
//
void WRCNCUI::draw_status_message(const bool blink) {
  ...
  void WRCNCUI::_set_brightness() { DWIN_LCD_Brightness(backlight ? brightness : 0); }
#endif
  ...
  #include "../../menu/menu.h"
    ...
    void WRCNCUI::draw_hotend_status(const uint8_t row, const uint8_t extruder) {
  ...
  void MenuEditItemBase::draw_edit_screen(PGM_P const pstr, const char* const value/*=nullptr*/) {
    ...
    if (vallen) {
      ...
      if (ui.currentScreen != _manual_move_func_ptr && !ui.external_control) {
        ...
        const dwin_coord_t slider_length = LCD_PIXEL_WIDTH - TERN(DWIN_WRCNCUI_LANDSCAPE, 120, 20),
                           slider_height = 16,
  ...
  inline void draw_boxed_string(const bool yesopt, PGM_P const pstr, const bool inv) {
    const uint8_t len = utf8_strlen_P(pstr),
                  mar = TERN(DWIN_WRCNCUI_PORTRAIT, 1, 4),
  ...
  void MenuItem_confirm::draw_select_screen(
    ...
    void WRCNCUI::ubl_plot(const uint8_t x_plot, const uint8_t y_plot) {
      ...
      lcd_moveto(
        TERN(DWIN_WRCNCUI_LANDSCAPE, ((x_offset + x_map_pixels) / MENU_FONT_WIDTH) + 2, 1),
        TERN(DWIN_WRCNCUI_LANDSCAPE, 1, ((y_offset + y_map_pixels) / MENU_LINE_HEIGHT) + 1)
      );
      lcd_put_u8str_P(X_LBL);
      lcd_put_u8str(ftostr52(lpos.x));
      lcd_moveto(
        TERN(DWIN_WRCNCUI_LANDSCAPE, ((x_offset + x_map_pixels) / MENU_FONT_WIDTH) + 2, 1),
        TERN(DWIN_WRCNCUI_LANDSCAPE, 3, ((y_offset + y_map_pixels) / MENU_LINE_HEIGHT) + 2)
      );
      ...
      dwin_string.add(")");
      lcd_moveto(
        TERN(DWIN_WRCNCUI_LANDSCAPE, ((x_offset + x_map_pixels) / MENU_FONT_WIDTH) + 2, LCD_WIDTH - dwin_string.length()),
        TERN(DWIN_WRCNCUI_LANDSCAPE, LCD_HEIGHT - 2, ((y_offset + y_map_pixels) / MENU_LINE_HEIGHT) + 1)
      );
      lcd_put_dwin_string();
      ...
      lcd_moveto(
        TERN(DWIN_WRCNCUI_LANDSCAPE, ((x_offset + x_map_pixels) / MENU_FONT_WIDTH) + 2, LCD_WIDTH - dwin_string.length()),
        TERN(DWIN_WRCNCUI_LANDSCAPE, LCD_HEIGHT - 1, ((y_offset + y_map_pixels) / MENU_LINE_HEIGHT) + 2)
      );
      lcd_put_dwin_string();
    }
    ...
    void WRCNCUI::zoffset_overlay(const int8_t dir) {
