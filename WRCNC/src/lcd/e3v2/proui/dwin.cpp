...
void DWIN_DrawStatusMessage() {
  const uint8_t max_status_chars = DWIN_WIDTH / DWINUI::fontWidth();
    ...
    if (slen <= max_status_chars) {
      ...
      uint8_t rlen;
      const char *stat = WRCNCUI::status_and_len(rlen);
 ...
 * and rely on WRCNC caching for performance. No need to
 * cache files here.
 */
...
void WRCNCUI::refresh() { /* Nothing to see here */ }
...
void DWIN_Redraw_screen() {
  ...
  void WRCNCUI::pause_show_message(const PauseMessage message, const PauseMode mode/*=PAUSE_MODE_SAME*/, const uint8_t extruder/*=active_extruder*/) {
    switch (message) {
