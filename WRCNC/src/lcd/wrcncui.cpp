...
#include "../WRCNCCore.h" // for printingIsPaused
...
// All displays share the WRCNCUI class
#include "wrcncui.h"
WRCNCUI ui;
  ...
  bool WRCNCUI::wait_for_move; // = false
#endif
...
constexpr uint8_t epps = ENCODER_PULSES_PER_STEP;
    ...
    uint8_t WRCNCUI::status_scroll_offset; // = 0
  #endif
  char WRCNCUI::status_message[MAX_MESSAGE_LENGTH + 1];
  uint8_t WRCNCUI::alert_level; // = 0
#endif

#if ENABLED(LCD_SET_PROGRESS_MANUALLY)
  WRCNCUI::progress_t WRCNCUI::progress_override; // = 0
  #if ENABLED(USE_M73_REMAINING_TIME)
    uint32_t WRCNCUI::remaining_time;
  #endif
#endif

#if HAS_MULTI_LANGUAGE
  uint8_t WRCNCUI::language; // Initialized by settings.load()
  void WRCNCUI::set_language(const uint8_t lang) {
    if (lang < NUM_LANGUAGES) {
      language = lang;
      TERN_(HAS_WRCNCUI_U8GLIB, update_language_font());
      return_to_status();
      refresh();
    }
  }
...
#if HAS_LCD_CONTRAST
  uint8_t WRCNCUI::contrast = LCD_CONTRAST_DEFAULT; // Initialized by settings.load()
  void WRCNCUI::set_contrast(const uint8_t value) {
  ...
  uint8_t WRCNCUI::brightness = LCD_BRIGHTNESS_DEFAULT;
  bool WRCNCUI::backlight = true;

  void WRCNCUI::set_brightness(const uint8_t value) {
    backlight = !!value;
  ...
  bool WRCNCUI::buzzer_enabled = true;
#endif
  ...
  #endif
  void WRCNCUI::buzz(const long duration, const uint16_t freq) {
  ...
  preheat_t WRCNCUI::material_preset[PREHEAT_COUNT];  // Initialized by settings.load()

  PGM_P WRCNCUI::get_preheat_label(const uint8_t m) {
  ...
  void WRCNCUI::apply_preheat(const uint8_t m, const uint8_t pmask, const uint8_t e/*=active_extruder*/) {
...
#if EITHER(HAS_WRCNCUI_MENU, EXTENSIBLE_UI)
  bool WRCNCUI::lcd_clicked;
#endif

#if EITHER(HAS_WIRED_LCD, DWIN_CREALITY_LCD_JYERSUI)

  bool WRCNCUI::get_blink() {
...
// Encoder Handling
  ...
  uint32_t WRCNCUI::encoderPosition;
...
void WRCNCUI::init() {
  ...
  #if HAS_ENCODER_ACTION
    volatile uint8_t WRCNCUI::buttons;
    #if HAS_SLOW_BUTTONS
      volatile uint8_t WRCNCUI::slow_buttons;
    #endif
    #if HAS_TOUCH_BUTTONS
      #include "touch/touch_buttons.h"
      bool WRCNCUI::on_edit_screen = false;
    #endif
  #endif

  #if SCREENS_CAN_TIME_OUT
    bool WRCNCUI::defer_return_to_status;
    millis_t WRCNCUI::return_to_status_ms = 0;
  #endif

  uint8_t WRCNCUI::lcd_status_update_delay = 1; // First update one loop delayed

  #if BOTH(FILAMENT_LCD_DISPLAY, SDSUPPORT)
    millis_t WRCNCUI::next_filament_display; // = 0
  #endif

  millis_t WRCNCUI::next_button_update_ms; // = 0

  #if HAS_WRCNCUI_U8GLIB
    bool WRCNCUI::drawing_screen, WRCNCUI::first_page; // = false
  #endif

  #if IS_DWIN_WRCNCUI
    bool WRCNCUI::did_first_redraw;
    bool WRCNCUI::old_is_printing;
  #endif

  #if ENABLED(SDSUPPORT)

    #if WRCNCUI_SCROLL_NAME
      uint8_t WRCNCUI::filename_scroll_pos, WRCNCUI::filename_scroll_max;
    #endif

    const char * WRCNCUI::scrolled_filename(CardReader &theCard, const uint8_t maxlen, uint8_t hash, const bool doScroll) {
    ...
    screenFunc_t WRCNCUI::currentScreen; // Initialized in CTOR
    bool WRCNCUI::screen_changed;

    #if ENABLED(ENCODER_RATE_MULTIPLIER)
      bool WRCNCUI::encoderRateMultiplierEnabled;
      millis_t WRCNCUI::lastEncoderMovementMillis = 0;
      void WRCNCUI::enable_encoder_multiplier(const bool onoff) {
      ...
      int8_t WRCNCUI::encoderDirection = ENCODERBASE;
    #endif

    #if HAS_TOUCH_BUTTONS
      uint8_t WRCNCUI::touch_buttons;
      uint8_t WRCNCUI::repeat_delay;
    #endif

    #if EITHER(AUTO_BED_LEVELING_UBL, G26_MESH_VALIDATION)

      bool WRCNCUI::external_control; // = false

      void WRCNCUI::wait_for_release() {
      ...
      void WRCNCUI::draw_select_screen_prompt(PGM_P const pref, const char * const string/*=nullptr*/, PGM_P const suff/*=nullptr*/) {
  ...
  ////////////////////////////////////////////
    ...
    volatile uint8_t WRCNCUI::keypad_buttons;

    #if HAS_WRCNCUI_MENU && !HAS_ADC_BUTTONS
   ...
   */
    ...
    millis_t WRCNCUI::progress_bar_ms; // = 0
    #if PROGRESS_MSG_EXPIRE > 0
      millis_t WRCNCUI::expire_status_ms; // = 0
    #endif
  #endif

  void WRCNCUI::status_screen() {

    TERN_(HAS_WRCNCUI_MENU, ENCODER_RATE_MULTIPLY(false));
  ...
  void WRCNCUI::kill_screen(FSTR_P const lcd_error, FSTR_P const lcd_component) {
    ...
    TERN_(HAS_WRCNCUI_MENU, return_to_status());
  ...
  void WRCNCUI::quick_feedback(const bool clear_buttons/*=true*/) {
    ...
    TERN_(HAS_WRCNCUI_MENU, refresh());
      ...
      #if BOTH(HAS_WRCNCUI_MENU, USE_BEEPER)
        for (int8_t i = 5; i--;) { buzzer.tick(); delay(2); }
      #elif HAS_WRCNCUI_MENU
        delay(10);
      #endif
    #endif
  }
  ...
  ////////////////////////////////////////////
    ...
    ManualMove WRCNCUI::manual_move{};
  ...
  LCDViewAction WRCNCUI::lcdDrawUpdate = LCDVIEW_CLEAR_CALL_REDRAW;
  ...
  void WRCNCUI::update() {
    ...
    if (ELAPSED(ms, next_lcd_update_ms) || TERN0(HAS_WRCNCUI_U8GLIB, drawing_screen)) {
      ...
      TERN_(LCD_HAS_STATUS_INDICATORS, update_indicators());
        ...
        if (encoderPastThreshold || lcd_clicked) {
          if (encoderPastThreshold && TERN1(IS_TFTGLCD_PANEL, !external_control)) {
            ...
            #if BOTH(HAS_WRCNCUI_MENU, ENCODER_RATE_MULTIPLIER)
      ...
      if (on_status_screen() && !lcd_status_update_delay--) {
        lcd_status_update_delay = TERN(HAS_WRCNCUI_U8GLIB, 12, 9);
        if (max_display_update_time) max_display_update_time--;  // Be sure never go to a very big number
        refresh(LCDVIEW_REDRAW_NOW);
      }

      #if BOTH(HAS_WRCNCUI_MENU, SCROLL_LONG_FILENAMES)
      ...
      if ((should_draw() || drawing_screen) && (!bbr2 || bbr2 > max_display_update_time)) {
        ...
        TERN_(HAS_ADC_BUTTONS, keypad_buttons = 0);
          ...
          TERN_(IS_DWIN_WRCNCUI, DWIN_UpdateLCD());

        #endif

        TERN_(HAS_WRCNCUI_MENU, lcd_clicked = false);
    ...
    void WRCNCUI::update_buttons() {
      ...
      if (ELAPSED(now, next_button_update_ms)) {
        ...
        if (enc != lastEncoderBits) {
          ...
          #if BOTH(HAS_WRCNCUI_MENU, AUTO_BED_LEVELING_UBL)
            external_encoder();
          #endif
          lastEncoderBits = enc;
        }
  ...
  bool WRCNCUI::has_status() { return (status_message[0] != '\0'); }

  void WRCNCUI::set_status(const char * const cstr, const bool persist) {
  ...
  void WRCNCUI::reset_status(const bool no_welcome) {
  ...
  void WRCNCUI::set_status(FSTR_P const fstr, int8_t level) {
  ...
  void WRCNCUI::set_alert_status(FSTR_P const fstr) {
    set_status(fstr, 1);
  ...
  void WRCNCUI::status_printf(const uint8_t level, FSTR_P const fmt, ...) {
    if (level < alert_level) return;
    alert_level = level;
    va_list args;
  ...
  void WRCNCUI::finish_status(const bool persist) {
    ...
    void WRCNCUI::advance_status_scroll() {
    ...
    char* WRCNCUI::status_and_len(uint8_t &len) {
  ...
  #if BOTH(HAS_WRCNCUI_MENU, PSU_CONTROL)

    void WRCNCUI::poweroff() {
  ...
  void WRCNCUI::flow_fault() {
    LCD_ALERTMESSAGE(MSG_FLOWMETER_FAULT);
    TERN_(HAS_BUZZER, buzz(1000, 440));
    TERN_(HAS_WRCNCUI_MENU, return_to_status());
  }

  void WRCNCUI::pause_print() {
    #if HAS_WRCNCUI_MENU
  ...
  void WRCNCUI::resume_print() {
    ...
    WRCNCUI::progress_t WRCNCUI::_get_progress() {
      return (
    ...
    //
    void WRCNCUI::screen_click(const uint8_t row, const uint8_t col, const uint8_t, const uint8_t) {
      const millis_t now = millis();
  ...
  //
  void WRCNCUI::set_status(const char * const cstr, const bool) {
    TERN(HOST_PROMPT_SUPPORT, hostui.notify(cstr), UNUSED(cstr));
  }
  void WRCNCUI::set_status(FSTR_P const fstr, const int8_t) {
    TERN(HOST_PROMPT_SUPPORT, hostui.notify(fstr), UNUSED(fstr));
  }
  void WRCNCUI::status_printf(const uint8_t, FSTR_P const fstr, ...) {
    TERN(HOST_PROMPT_SUPPORT, hostui.notify(fstr), UNUSED(fstr));
  }
  ...
  void WRCNCUI::media_changed(const uint8_t old_status, const uint8_t status) {
  ...
  void WRCNCUI::reset_settings() {
    ...
    void WRCNCUI::zoffset_overlay(const_float_t zvalue) {
  ...
  void WRCNCUI::pause_show_message(
    ...
    void WRCNCUI::eeprom_alert(const uint8_t msgid) {
      #if HAS_WRCNCUI_MENU
