...
//
enum WRCNCDebugFlags : uint8_t {
  WRCNC_DEBUG_NONE          = 0,
  WRCNC_DEBUG_ECHO          = _BV(0), ///< Echo commands in order as they are processed
  WRCNC_DEBUG_INFO          = _BV(1), ///< Print messages for code that has debug output
  WRCNC_DEBUG_ERRORS        = _BV(2), ///< Not implemented
  WRCNC_DEBUG_DRYRUN        = _BV(3), ///< Ignore temperature setting and E movement commands
  WRCNC_DEBUG_COMMUNICATION = _BV(4), ///< Not implemented
  #if ENABLED(DEBUG_LEVELING_FEATURE)
    WRCNC_DEBUG_LEVELING    = _BV(5), ///< Print detailed output for homing and leveling
    WRCNC_DEBUG_MESH_ADJUST = _BV(6), ///< UBL bed leveling
  #else
    WRCNC_DEBUG_LEVELING    = 0,
    WRCNC_DEBUG_MESH_ADJUST = 0,
  #endif
  WRCNC_DEBUG_ALL           = 0xFF
};

extern uint8_t wrcnc_debug_flags;
#define DEBUGGING(F) (wrcnc_debug_flags & (WRCNC_DEBUG_## F))
