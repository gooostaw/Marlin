...
#define DEBUG_OUT EITHER(DEBUG_CARDREADER, WRCNC_DEV_MODE)
...
//
// Return 'true' if the item is something WRCNC can read
//
...
void CardReader::mount() {
  ...
  if (flag.mounted)
    ...
    else if (wrcnc_state != MF_INITIALIZING)
...
/**
 * Open a G-code file and set WRCNC to start processing it.
...
void CardReader::fileHasFinished() {
  ...
  wrcnc_state = MF_SD_COMPLETE;  // Tell WRCNC to enqueue M1001 soon
}
