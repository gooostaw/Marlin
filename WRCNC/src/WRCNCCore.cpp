...
#if HAS_L64XX
  #include "libs/L64XX/L64XX_WRCNC.h"
#endif
...
WRCNCState wrcnc_state = MF_INITIALIZING;
...
void startOrResumeJob() {
  ...
  inline void finishSDPrinting() {
    if (queue.enqueue_one(F("M1001"))) {  // Keep trying until it gets queued
      wrcnc_state = MF_RUNNING;          // Signal to stop trying
...
/**
 * Minimal management of WRCNC's core activities:
    ...
    #define _CHECK_CUSTOM_USER_BUTTON(N, CODE) do{                     \
      ...
      if (BUTTON##N##_HIT_STATE == READ(BUTTON##N##_PIN)               \
        ...
        if (ELAPSED(ms, next_cub_ms_##N)) {                            \
          ...
          TERN_(HAS_WRCNCUI_MENU, ui.quick_feedback());                    \
        }                                                              \
      }                                                                \
    }while(0)
...
void idle(bool no_stepper_sleep/*=false*/) {
  ...
  if (wrcnc_state == MF_INITIALIZING) goto IDLE_DONE;
  ...
  IDLE_DONE:
  TERN_(WRCNC_DEV_MODE, idle_depth--);
  return;
}
...
/**
 * Webber Ranch CNC Firmware entry-point. Abandon Hope All Ye Who Enter Here.
 ...
 *  - Install WRCNC custom Exception Handlers, if set.
 *  - Init WRCNC's HAL interfaces (for SPI, i2c, etc.)
...
void setup() {
  ...
  TERN_(DYNAMIC_VECTORTABLE, hook_cpu_exceptions()); // If supported, install WRCNC exception handlers at runtime
  ...
  // Identify myself as WRCNC x.x.x
  SERIAL_ECHOLNPGM("WRCNC " SHORT_BUILD_VERSION);
  ...
  #if BOTH(HAS_WRCNCUI_MENU, TOUCH_SCREEN_CALIBRATION) && EITHER(TFT_CLASSIC_UI, TFT_COLOR_UI)
...
void loop() {
  do {
    idle();
      ...
      if (wrcnc_state == MF_SD_COMPLETE) finishSDPrinting();
    #endif
