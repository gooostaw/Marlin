 ...
 */
  ...
  #define EEPROM_SLOTS            ((FLASH_UNIT_SIZE) / (WRCNC_EEPROM_SIZE))
  #define SLOT_ADDRESS(slot)      (FLASH_ADDRESS_START + (slot * (WRCNC_EEPROM_SIZE)))
  ...
  static uint8_t ram_eeprom[WRCNC_EEPROM_SIZE] __attribute__((aligned(4))) = {0};
  static int current_slot = -1;

  static_assert(0 == WRCNC_EEPROM_SIZE % 4, "WRCNC_EEPROM_SIZE must be a multiple of 4"); // Ensure copying as uint32_t is safe
  static_assert(0 == FLASH_UNIT_SIZE % WRCNC_EEPROM_SIZE, "WRCNC_EEPROM_SIZE must divide evenly into your FLASH_UNIT_SIZE");
  static_assert(FLASH_UNIT_SIZE >= WRCNC_EEPROM_SIZE, "FLASH_UNIT_SIZE must be greater than or equal to your WRCNC_EEPROM_SIZE");
...
#ifndef WRCNC_EEPROM_SIZE
  #define WRCNC_EEPROM_SIZE size_t(E2END + 1)
#endif
    ...
    if (current_slot == -1 || eeprom_data_written) {
      ...
      while (address <= FLASH_ADDRESS_END) {
        ...
        if (address_value != EMPTY_UINT32) {
          current_slot = (address - (FLASH_ADDRESS_START)) / (WRCNC_EEPROM_SIZE);
          break;
        }
      ...
      if (current_slot == -1) {
        ...
        for (int i = 0; i < WRCNC_EEPROM_SIZE; i++) ram_eeprom[i] = EMPTY_UINT8;
        current_slot = EEPROM_SLOTS;
      }
      else {
        // load current settings
        uint8_t *eeprom_data = (uint8_t *)SLOT_ADDRESS(current_slot);
        for (int i = 0; i < WRCNC_EEPROM_SIZE; i++) ram_eeprom[i] = eeprom_data[i];
...
bool PersistentStore::access_finish() {
  ...
  if (eeprom_data_written) {
      ...
      uint32_t address_end = address + WRCNC_EEPROM_SIZE;
      uint32_t data = 0;
