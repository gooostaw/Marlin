...
USBMassStorage WRCNCMSC;
Serial1Class<USBCompositeSerial> WRCNCCompositeSerial(true);
...
#include "../../inc/WRCNCConfig.h"
  ...
  void my_rx_callback(unsigned int, void*) {
    ...
    for (uint32 i = 0; i < len; i++)
      emergency_parser.update(WRCNCCompositeSerial.emergency_state, buf[i+total-len]);
  }
...
void MSC_SD_init() {
  ...
  // Just set WRCNCCompositeSerial enabled to true
  // because when WRCNCCompositeSerial.begin() is used in setup()
  // it clears all USBComposite devices.
  WRCNCCompositeSerial.begin();
  USBComposite.end();
  USBComposite.clear();
  // Set api and register mass storage
    ...
    if (disk_initialize(0) == RES_OK) {
      if (disk_ioctl(0, GET_SECTOR_COUNT, (void *)(&cardSize)) == RES_OK) {
        WRCNCMSC.setDriveData(0, cardSize, MSC_Read, MSC_Write);
        WRCNCMSC.registerComponent();
      }
    }
  #endif
  // Register composite Serial
  WRCNCCompositeSerial.registerComponent();
  USBComposite.begin();
  #if ENABLED(EMERGENCY_PARSER)
