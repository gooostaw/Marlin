...
// Store settings in the last two pages
  ...
  #define WRCNC_EEPROM_SIZE ((EEPROM_PAGE_SIZE) * 2)
#endif
...
bool PersistentStore::access_start() {
  ...
  static_assert(0 == (WRCNC_EEPROM_SIZE) % 4, "WRCNC_EEPROM_SIZE is corrupted. (Must be a multiple of 4.)"); // Ensure copying as uint32_t is safe
  constexpr size_t eeprom_size_u32 = (WRCNC_EEPROM_SIZE) / 4;
...
bool PersistentStore::access_finish() {
  ...
  if (eeprom_dirty) {
    ...
    for (size_t i = 0; i < WRCNC_EEPROM_SIZE; i += 2, ++source) {
