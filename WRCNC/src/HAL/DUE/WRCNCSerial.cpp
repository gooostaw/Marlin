...
/**
 * WRCNCSerial_Due.cpp - Hardware serial library for Arduino DUE
 * Copyright (c) 2017 Eduardo José Tagle. All right reserved
 * Based on WRCNCSerial for AVR, copyright (c) 2006 Nicholas Zambetti.  All right reserved.
 */
...
template<typename Cfg>
FORCE_INLINE void WRCNCSerial<Cfg>::_tx_thr_empty_irq() {
  if (Cfg::TX_SIZE > 0) {
    // Read positions
...
// Public Methods
template<typename Cfg>
void WRCNCSerial<Cfg>::begin(const long baud_setting) {
...
template<typename Cfg>
size_t WRCNCSerial<Cfg>::write(const uint8_t c) {
  _written = true;
...
// If not using the USB port as serial port
  ...
  MSerialT1 customizedSerial1(WRCNCSerialCfg<SERIAL_PORT>::EMERGENCYPARSER);
#endif

#if defined(SERIAL_PORT_2) && SERIAL_PORT_2 >= 0
  template class WRCNCSerial< WRCNCSerialCfg<SERIAL_PORT_2> >;
  MSerialT2 customizedSerial2(WRCNCSerialCfg<SERIAL_PORT_2>::EMERGENCYPARSER);
#endif

#if defined(SERIAL_PORT_3) && SERIAL_PORT_3 >= 0
  template class WRCNCSerial< WRCNCSerialCfg<SERIAL_PORT_3> >;
  MSerialT3 customizedSerial3(WRCNCSerialCfg<SERIAL_PORT_3>::EMERGENCYPARSER);
#endif
