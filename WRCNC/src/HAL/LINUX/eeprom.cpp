...
#include <stdio.h>
  ...
  #define WRCNC_EEPROM_SIZE 0x1000 // 4KB of Emulated EEPROM
#endif

uint8_t buffer[WRCNC_EEPROM_SIZE];
...
bool PersistentStore::access_start() {
  ...
  if (file_size < WRCNC_EEPROM_SIZE) {
    memset(buffer + file_size, eeprom_erase_value, WRCNC_EEPROM_SIZE - file_size);
  }
  else {
