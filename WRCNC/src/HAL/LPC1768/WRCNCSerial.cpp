...
#include "../../inc/WRCNCConfig.h"
  ...
  bool WRCNCSerial::recv_callback(const char c) {
    // Need to figure out which serial port we are and react in consequence (WRCNC does not have CONTAINER_OF macro)
    if (false) {}
    #if USING_HW_SERIAL0
