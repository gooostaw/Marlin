...
#define EEPROM_SLOTS ((SECTOR_SIZE)/(WRCNC_EEPROM_SIZE))
#define EEPROM_ERASE 0xFF
#define SLOT_ADDRESS(sector, slot) (((uint8_t *)SECTOR_START(sector)) + slot * (WRCNC_EEPROM_SIZE))
...
bool PersistentStore::access_start() {
  ...
  if (status == CMD_SUCCESS) {
    ...
    for (int i = 0; i < WRCNC_EEPROM_SIZE; i++) ram_eeprom[i] = EEPROM_ERASE;
    current_slot = EEPROM_SLOTS;
  }
  else {
    // current slot is the first non blank one
    current_slot = first_nblank_loc / (WRCNC_EEPROM_SIZE);
