...
void HAL_init() {
  ...
  // Init LEDs
    ...
    // Flash status LED 3 times to indicate WRCNC has started booting
    LOOP_L_N(i, 6) {
      TOGGLE(LED_PIN);
      delay(100);
    }
...
void HAL_idletask() {
  #if HAS_SHARED_MEDIA
    // If WRCNC is using the SD card we need to lock it to prevent access from
    // a PC via USB.
    // Other HALs use IS_SD_PRINTING() and IS_SD_FILE_OPEN() to check for access but
    // this will not reliably detect delete operations. To be safe we will lock
    // the disk if WRCNC has it mounted. Unfortunately there is currently no way
