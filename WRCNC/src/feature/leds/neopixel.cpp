...
WRCNC_NeoPixel neo;
pixel_index_t WRCNC_NeoPixel::neoindex;

Adafruit_NeoPixel WRCNC_NeoPixel::adaneo1(NEOPIXEL_PIXELS, NEOPIXEL_PIN, NEOPIXEL_TYPE + NEO_KHZ800);
#if CONJOINED_NEOPIXEL
  Adafruit_NeoPixel WRCNC_NeoPixel::adaneo2(NEOPIXEL_PIXELS, NEOPIXEL2_PIN, NEOPIXEL2_TYPE + NEO_KHZ800);
#endif

#ifdef NEOPIXEL_BKGD_INDEX_FIRST

  void WRCNC_NeoPixel::set_background_color(uint8_t r, uint8_t g, uint8_t b, uint8_t w) {
  ...
  void WRCNC_NeoPixel::reset_background_color() {
...
void WRCNC_NeoPixel::set_color(const uint32_t color) {
  if (neoindex >= 0) {
...
void WRCNC_NeoPixel::set_color_startup(const uint32_t color) {
  for (uint16_t i = 0; i < pixels(); ++i)
    set_pixel_color(i, color);
  show();
}

void WRCNC_NeoPixel::init() {
  ...
  WRCNC_NeoPixel2 neo2;

  pixel_index_t WRCNC_NeoPixel2::neoindex;
  Adafruit_NeoPixel WRCNC_NeoPixel2::adaneo(NEOPIXEL2_PIXELS, NEOPIXEL2_PIN, NEOPIXEL2_TYPE);

  void WRCNC_NeoPixel2::set_color(const uint32_t color) {
    if (neoindex >= 0) {
      set_pixel_color(neoindex, color);
      neoindex = -1;
    }
  ...
  void WRCNC_NeoPixel2::set_color_startup(const uint32_t color) {
    for (uint16_t i = 0; i < pixels(); ++i)
      set_pixel_color(i, color);
    show();
  }

  void WRCNC_NeoPixel2::init() {
