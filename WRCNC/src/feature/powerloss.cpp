...
void CNCJobRecovery::save(const bool force/*=false*/, const float zraise/*=POWER_LOSS_ZRAISE*/, const bool raised/*=false*/) {
  ...
  if (force
    ...
    // Misc. WRCNC flags
    info.flag.dryrun = !!(wrcnc_debug_flags & WRCNC_DEBUG_DRYRUN);
...
void CNCJobRecovery::resume() {
  ...
  if (info.flag.dryrun) wrcnc_debug_flags |= WRCNC_DEBUG_DRYRUN;
  ...
  gcode.axis_relative = info.axis_relative;
    ...
    const uint8_t old_flags = wrcnc_debug_flags;
    wrcnc_debug_flags |= WRCNC_DEBUG_ECHO;
  #endif
  ...
  TERN_(DEBUG_POWER_LOSS_RECOVERY, wrcnc_debug_flags = old_flags);
}
