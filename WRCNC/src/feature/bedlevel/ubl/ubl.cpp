...
void unified_bed_leveling::report_current_mesh() {
  ...
  GRID_LOOP(x, y)
    if (!isnan(z_values[x][y])) {
      ...
      serial_delay(75); // Prevent CNCrun from exploding
    }
}
  ...
  void GcodeSuite::M1004() {
    ...
    #define PROBE_GCODE TERN(HAS_BED_PROBE, "G29P1\nG29P3", "G29P4R")
      ...
      if (parser.seenval('H')) {                          // Handle H# parameter to set Hotend temp
        const celsius_t hotend_temp = parser.value_int(); // WRCNC never sends itself F or K, always C
      ...
      if (parser.seenval('B')) {                        // Handle B# parameter to set Bed temp
        const celsius_t bed_temp = parser.value_int();  // WRCNC never sends itself F or K, always C
