...
#define CHOPPER_WRCNC_119   { 5,  2, 3 }
...
class TMCWRCNC : public TMC, public TMCStorage<AXIS_LETTER, DRIVER_ID> {
  public:
    TMCWRCNC(const uint16_t cs_pin, const float RS) :
      TMC(cs_pin, RS)
      {}
    TMCWRCNC(const uint16_t cs_pin, const float RS, const uint8_t axis_chain_index) :
      TMC(cs_pin, RS, axis_chain_index)
      {}
    TMCWRCNC(const uint16_t CS, const float RS, const uint16_t pinMOSI, const uint16_t pinMISO, const uint16_t pinSCK) :
      TMC(CS, RS, pinMOSI, pinMISO, pinSCK)
      {}
    TMCWRCNC(const uint16_t CS, const float RS, const uint16_t pinMOSI, const uint16_t pinMISO, const uint16_t pinSCK, const uint8_t axis_chain_index) :
    ...
    void set_chopper_times(const chopper_timing_t &ct) {
      ...
      void set_pwm_thrs(const uint32_t thrs) {
        ...
        TERN_(HAS_WRCNCUI_MENU, this->stored.hybrid_thrs = thrs);
      }
    #endif
      ...
      void homing_threshold(int16_t sgt_val) {
        ...
        TMC::sgt(sgt_val);
        TERN_(HAS_WRCNCUI_MENU, this->stored.homing_thrs = sgt_val);
      }
...
class TMCWRCNC<TMC2208Stepper, AXIS_LETTER, DRIVER_ID, AXIS_ID> : public TMC2208Stepper, public TMCStorage<AXIS_LETTER, DRIVER_ID> {
  public:
    TMCWRCNC(Stream * SerialPort, const float RS, const uint8_t) :
      TMC2208Stepper(SerialPort, RS)
      {}
    TMCWRCNC(Stream * SerialPort, const float RS, uint8_t addr, const uint16_t mul_pin1, const uint16_t mul_pin2) :
      TMC2208Stepper(SerialPort, RS, addr, mul_pin1, mul_pin2)
     {}
    TMCWRCNC(const uint16_t RX, const uint16_t TX, const float RS, const uint8_t) :
...
template<char AXIS_LETTER, char DRIVER_ID, AxisEnum AXIS_ID>
class TMCWRCNC<TMC2209Stepper, AXIS_LETTER, DRIVER_ID, AXIS_ID> : public TMC2209Stepper, public TMCStorage<AXIS_LETTER, DRIVER_ID> {
  public:
    TMCWRCNC(Stream * SerialPort, const float RS, const uint8_t addr) :
      TMC2209Stepper(SerialPort, RS, addr)
      {}
    TMCWRCNC(const uint16_t RX, const uint16_t TX, const float RS, const uint8_t addr) :
...
template<char AXIS_LETTER, char DRIVER_ID, AxisEnum AXIS_ID>
class TMCWRCNC<TMC2660Stepper, AXIS_LETTER, DRIVER_ID, AXIS_ID> : public TMC2660Stepper, public TMCStorage<AXIS_LETTER, DRIVER_ID> {
  public:
    TMCWRCNC(const uint16_t cs_pin, const float RS, const uint8_t) :
      TMC2660Stepper(cs_pin, RS)
      {}
    TMCWRCNC(const uint16_t CS, const float RS, const uint16_t pinMOSI, const uint16_t pinMISO, const uint16_t pinSCK, const uint8_t) :
 ...
 */
  ...
  void tmc_disable_stallguard(TMC2660Stepper, const bool);
    ...
    template<class TMC, char AXIS_LETTER, char DRIVER_ID, AxisEnum AXIS_ID>
    bool TMCWRCNC<TMC, AXIS_LETTER, DRIVER_ID, AXIS_ID>::test_stall_status() {
