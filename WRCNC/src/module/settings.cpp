...
//static_assert(sizeof(SettingsData) <= WRCNC_EEPROM_SIZE, "EEPROM too small to contain SettingsData!");

WRCNCSettings settings;

uint16_t WRCNCSettings::datasize() { return sizeof(SettingsData); }
...
void WRCNCSettings::postprocess() {
...
#include "../core/debug_out.h"
  ...
  bool WRCNCSettings::eeprom_error, WRCNCSettings::validating;
  int WRCNCSettings::eeprom_index;
  uint16_t WRCNCSettings::working_crc;

  bool WRCNCSettings::size_error(const uint16_t size) {
    if (size != datasize()) {
  ...
  bool WRCNCSettings::_load() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      ...
      DEBUG_ECHO_MSG("EEPROM version mismatch (EEPROM=", stored_ver, " WRCNC=" EEPROM_VERSION ")");
  ...
  bool WRCNCSettings::load() {
    ...
    const uint16_t WRCNCSettings::meshes_end = persistentStore.capacity() - 129;

    uint16_t WRCNCSettings::meshes_start_index() {
    ...
    uint16_t WRCNCSettings::calc_num_meshes() {
      return (meshes_end - meshes_start_index()) / MESH_STORE_SIZE;
    }

    int WRCNCSettings::mesh_slot_offset(const int8_t slot) {
      return meshes_end - (slot + 1) * MESH_STORE_SIZE;
    }

    void WRCNCSettings::store_mesh(const int8_t slot) {
    ...
    void WRCNCSettings::load_mesh(const int8_t slot, void * const into/*=nullptr*/) {
    ...
    //void WRCNCSettings::delete_mesh() { return; }
    //void WRCNCSettings::defrag_meshes() { return; }
...
void WRCNCSettings::reset() {
   ...
   */
  void WRCNCSettings::report(const bool forReplay) {
    //
