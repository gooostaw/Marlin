...
#if ENABLED(G26_MESH_VALIDATION) && EITHER(HAS_WRCNCUI_MENU, EXTENSIBLE_UI)
...
class Temperature {
  ...
  public:
    ...
    static bool targetHotEnoughToExtrude(const uint8_t e) { return !targetTooColdToExtrude(e); }
      ...
      // Heater idle handling. WRCNC creates one per hotend and one for the heated bed.
      typedef struct {
        millis_t timeout_ms;
        bool timed_out;
  ...
  public:
    ...
    #if HAS_WRCNCUI_MENU && HAS_TEMPERATURE
