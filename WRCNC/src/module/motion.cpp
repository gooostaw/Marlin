...
inline void report_machine_position(const xyze_pos_t &rpos) {
  ...
  void report_compact_status(const xyze_pos_t &rpos) {
    ...
    SERIAL_ECHOPGM("|", feedrate_percentage);                     // Feed rate % (M220)
      ...
      SERIAL_ECHOLNPGM("|", grbl_state_for_wrcnc_state());       // GRBL compatible status (state) eg: M_IDLE
    #else
      SERIAL_ECHOLNPGM("|", wrcnc_state);                        // WRCNC status (state) eg: MF_WAITING
    #endif
  }
#endif
...
void report_current_position_projected() {
  ...
  /**
   * Set a Grbl-compatible state from the current wrcnc_state
   */
  M_StateEnum grbl_state_for_wrcnc_state() {
    switch (wrcnc_state) {
...
void prepare_line_to_destination() {
  ...
  void set_axis_never_homed(const AxisEnum axis) {
    ...
    void backout_to_tmc_homing_phase(const AxisEnum axis) {
      ...
      int16_t phasePerUStep,      // TMC µsteps(phase) per WRCNC µsteps
      ...
      // Convert TMC µsteps(phase) to whole WRCNC µsteps to effector backout direction to mm
