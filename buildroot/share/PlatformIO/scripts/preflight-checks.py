...
if pioutil.is_pio_build():
	...
	def get_envs_for_board(board):
		with open(os.path.join("mvCNC", "src", "pins", "pins.h"), "r") as file:
	...
	def sanity_check_target():
		...
		for p in [ env['PROJECT_DIR'], os.path.join(env['PROJECT_DIR'], "config") ]:
			for f in [ "Configuration.h", "Configuration_adv.h" ]:
				if os.path.isfile(os.path.join(p, f)):
					err = "ERROR: Config files found in directory %s. Please move them into the mvCNC subfolder." % p
					raise SystemExit(err)
		...
		#
		# Check for old files indicating an entangled mvCNC (mixing old and new code)
		#
		mixedin = []
		p = os.path.join(env['PROJECT_DIR'], "mvCNC", "src", "lcd", "dogm")
		for f in [ "ultralcd_DOGM.cpp", "ultralcd_DOGM.h" ]:
			if os.path.isfile(os.path.join(p, f)):
				mixedin += [ f ]
		if mixedin:
			err = "ERROR: Old files fell into your mvCNC folder. Remove %s and try again" % ", ".join(mixedin)
			raise SystemExit(err)
