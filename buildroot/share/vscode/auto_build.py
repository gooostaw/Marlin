...
#
#  1. Scans Configuration.h for the motherboard name and mvCNC version.
...
#   return: board name, major version of mvCNC being used (1 or 2)
def get_board_name():
  board_name = ''
  # get board name

  with open('mvCNC/Configuration.h', 'r') as myfile:
    Configuration_h = myfile.read()

  Configuration_h = Configuration_h.split('\n')
  mvCNC_ver = 0  # set version to invalid number
...
#  return: environment
def get_env(board_name, ver_mvCNC):
  ...
  possible_envs = get_starting_env(board_name, ver_mvCNC)
  ...
  if build_type == 'traceback' and target_env != 'LPC1768_debug_and_upload' and target_env != 'DUE_debug' and mvCNC_ver == 2:
...
# text editing section
  ...
  def _rebuild(self):
    ...
    #global mvCNC_ver
    #global target_env
    #board_name, mvCNC_ver = get_board_name()
    #target_env = get_env(board_name, mvCNC_ver)
    self.start_thread()
